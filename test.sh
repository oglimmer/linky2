...
if test $argv[1] = "create-user-rnd";
  curl -s $BASE_URL/v1/users -d '{"email": "'$USER_EMAIL(cat /dev/random | LC_CTYPE=C tr -dc "[:alpha:]" | head -c 8)'", "password": "'$USER_PASSWORD'"}' -H 'Content-Type: application/json'|jq
end

if test $argv[1] = "auth";
  #echo "curl -s $BASE_URL/v1/users/auth -d '{"email": "'$USER_EMAIL'", "password": "'$USER_PASSWORD'"}' -H 'Content-Type: application/json'"
